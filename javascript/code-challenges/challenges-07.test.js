...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    let startIndex=element.indexOf(" ",4)
    result.push(element.substring(startIndex,element.length).trim())
  });
  return result;
  return result;
};
...
const removeEvenValues = (arr) => {
  // Solution code here...
  str.forEach(element => {
    element % 2 === 0 ? str.splice(str.indexOf(element),1) :''
  });
};
