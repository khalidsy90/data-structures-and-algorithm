...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let uniqueValues=arr.filter(num =>{
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let justNames=arr.filter(item => {
    return item.baseStat > minBaseStat;
  }).map(names =>{
    return names.stat.name;
  });
  return justNames;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let numericValues=arr.filter(item =>{
